		...
		return list.map((vnode) => {
			if (vnode === "info") {
				...
				if (typeof vnode === "object") {
					...
					if (vnode.children && isArray(vnode.children) && vnode.children.length > 0) {
						return (
							<el-dropdown trigger="click" onCommand={(key: string) => {
								const item = vnode.children.find((e: any) => e.label === key || e.key === key);
								...
								{{
									...
									dropdown: () => (
										<el-dropdown-menu>
											{vnode.children.map((child: any, idx: number) => (
												<el-dropdown-item
													key={child.key || child.label || idx}
					...
					custom(vnode) {
						return (
							<el-button
								...
								{...vnode?.props}
								onClick={(e: MouseEvent) => {
									if (typeof vnode.onClick === "function") {
										vnode.onClick({ scope });
									}
									e.stopPropagation();
								}}>
