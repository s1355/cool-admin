...
router.find = function (path: string) {
	...
	// 如果匹配到 catch-all 路由，检查模块页面是否有更精确匹配
	if (matchedRoute && matchedRoute.path === '/:catchAll(.*)' && path !== '/') {
		const specificRoute = routeList.find(
			r => r.path === path && r.path !== '/:catchAll(.*)'
		);
		if (specificRoute) {
			return { route: specificRoute, isReg: !!specificRoute.isReg };
		}
	}
...
router.beforeEach(async (to, from, next) => {
	...
	// demo 模式：访问 /login 时清除旧 token，确保能进入登录页
	if (import.meta.env.MODE === 'demo' && to.path.includes('/login')) {
		user.clear();
		next();
		return;
	}
