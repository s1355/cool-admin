<template>
	<div class="book-cover-edit">
		<!-- 已有封面列表 -->
		<div v-if="covers.length > 0" class="cover-list">
			<div v-for="(url, index) in covers" :key="url + index" class="cover-item">
				<el-image
					class="cover-img"
					:src="url"
					fit="cover"
		...
		<!-- 添加按钮 -->
		<div class="cover-add" @click="showAddDialog">
		...
		<el-dialog
			...
			:title="editingIndex === -1 ? '添加封面' : '编辑封面'"
			width="500px"
			destroy-on-close
		>
			<el-form label-width="80px">
				...
				<template v-else>
					...
					<el-form-item v-if="localFile" label="预览">
						<img :src="localFilePreview" alt="预览" class="cover-preview" />
					</el-form-item>
				</template>
			</el-form>
...
// 将封面数据统一转为字符串数组（兼容旧格式：`[{"url":"..."}]`）
function normalizeCovers(val: any[]): string[] {
	return (val || []).map((c: any) => (typeof c === 'object' && c?.url ? c.url : String(c)));
}

// 封面列表
const covers = ref<string[]>(normalizeCovers(props.modelValue));

// 监听外部值变化
watch(
	() => props.modelValue,
	(val) => {
		covers.value = normalizeCovers(val);
	},
...
// 能否添加
const canAdd = computed(() => covers.value.length < props.limit);
...
function showAddDialog() {
	if (!canAdd.value) {
		ElMessage.warning(`最多只能添加 ${props.limit} 张封面`);
		return;
	}
...
// 编辑封面
function editCover(index: number) {
	editingIndex.value = index;
	inputUrl.value = covers.value[index];
	inputUrlError.value = false;
	localFile.value = null;
	localFilePreview.value = '';
	addMode.value = 'url';
...
async function confirmAdd() {
	if (addMode.value === 'url') {
		...
		if (editingIndex.value === -1) {
			covers.value.push(url);
		} else {
			covers.value[editingIndex.value] = url;
		}
	} else {
		if (localFile.value) {
			ElMessage.info('正在上传...');
...
function updateValue() {
	emit('update:modelValue', [...covers.value]);
}
