...
// API: service.knowledge.bookCategory - 书籍分类服务（由 EPS 自动生成）
// 对应后端路由 /admin/knowledge/book-category

// cl-crud 配置
const Crud = useCrud({ service: service.knowledge.bookCategory }, app => {
	// 首次加载时刷新数据
	app.refresh();
});
				...
				},
				{
					...
					onClick({ scope }: any) {
						ElMessageBox.confirm('确定要删除该分类吗？', '提示', {
							...
							service.knowledge.bookCategory.delete({ ids: [scope.row.id] }).then(() => {
...
function handleMultiDelete() {
	...
	ElMessageBox.confirm(`确定要删除选中的 ${ids.length} 个分类吗？`, '提示', {
		...
		service.knowledge.bookCategory.delete({ ids }).then(() => {
			ElMessage.success('删除成功');
			Crud.value?.refresh();
		}).catch((err: any) => {
